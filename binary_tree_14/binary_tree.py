...
class BST:
    ...
    def DeleteNodeByKey(self, key):
        # TODO: EN doc
        # удаляемый узел надо заменить так называемым
        # узлом-преемником, ключ которого -- наименьший из всех
        # ключей, которые больше ключа удаляемого узла.
        # Иными словами, нам надо взять правого потомка удаляемого
        # узла, и далее спускаться по всем левым потомкам. Если мы
        # находим лист, то его и надо поместить вместо удаляемого
        # узла. Если мы находим узел, у которого есть только
        # правый потомок, то преемником берём этого правого потомка.
